def generate_triangle_sequence(n):
    ...
    A Triangle Number is a number that can be represented as a triangular 
    grid of points where the first row contains a single element and 
    each subsequent row contains one more element than the previous row.

    The sequence is defined by the formula: T(n) = n * (n + 1) / 2
